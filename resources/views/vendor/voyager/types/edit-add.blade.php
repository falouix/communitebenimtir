...
<div class="page-content container-fluid">
    <form class="form-edit-add" role="form"
        action="@if(isset($dataTypeContent->id)){{ route('voyager.types.update', $dataTypeContent->id) }}@else{{ route('voyager.types.store') }}@endif"
                    ...
                    <div class="tab-content">
                        <div id="FR" class="tab-pane fade active in">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="type_fr" name="type_fr"
                                    placeholder="Le titre" value="{{ $dataTypeContent->type_fr ?? '' }}"
                                    autocomplete="off">
                            </div>
                            <div class="form-group">
                                <label for="slug">Nature de texte</label>
                            
                                @php
                                $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows' )};
                                $row = $dataTypeRows->where('field', 'type_loi_fr')->first();
                                @endphp
                                {!! app('voyager')->formField($row, $dataType, $dataTypeContent) !!}
                            </div>
                            <div class="form-group">
                                <label for="slug">Les textes juriqiques</label>

                                @php
                                $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows'
                                )};
                                $row = $dataTypeRows->where('field', 'texte_juridique_fr')->first();
                                @endphp
                                {!! app('voyager')->formField($row, $dataType, $dataTypeContent) !!}
                            </div>
                        </div>
                        <div id="AR" class="tab-pane fade " style="direction: rtl;">
                            <!-- ### TITLE ### -->
                            <div class="form-group">
                                <label for="slug">عنوان النص</label>
                                <input type="text" class="form-control" id="type_ar" name="type_ar"
                                    placeholder="اكتب عنوان النص"
                                    value="{{ $dataTypeContent->type_ar ?? '' }}" autocomplete="off">
                            </div>
                            <div class="form-group">
                                <label for="slug">طبيعة النص</label>
                            
                                @php
                                $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows' )};
                                $row = $dataTypeRows->where('field', 'type_loi_ar')->first();
                                @endphp
                                {!! app('voyager')->formField($row, $dataType, $dataTypeContent) !!}
                            </div>
                            <div class="form-group">
                                <label for="slug">النصوص القانونية</label>
                                @php
                                $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows'
                                )};
                                $row = $dataTypeRows->where('field', 'texte_juridique_ar')->first();
                                @endphp
                                {!! app('voyager')->formField($row, $dataType, $dataTypeContent) !!}
                            </div>
                        </div>
                        <div id="EN" class="tab-pane fade">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="type_en" name="type_en"
                                    placeholder="The title" value="{{ $dataTypeContent->type_en ?? '' }}"
                                    autocomplete="off">
                            </div>
                            <div class="form-group">
                                <label for="slug">Text type</label>
                            
                                @php
                                $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows' )};
                                $row = $dataTypeRows->where('field', 'type_loi_en')->first();
                                @endphp
                                {!! app('voyager')->formField($row, $dataType, $dataTypeContent) !!}
                            </div>
                            <div class="form-group">
                                <label for="slug">Legal texts</label>
                                @php
                                $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows'
                                )};
                                $row = $dataTypeRows->where('field', 'texte_juridique_en')->first();
                                @endphp
                                {!! app('voyager')->formField($row, $dataType, $dataTypeContent) !!}
                            </div>
                        </div>
