...
<div class="page-content container-fluid">
    <form class="form-edit-add" role="form"
        action="@if(isset($dataTypeContent->id)){{ route('voyager.finances.update', $dataTypeContent->id) }}@else{{ route('voyager.finances.store') }}@endif"
                    ...
                    <div class="tab-content">
                        ...
                        <div id="images" class="tab-pane fade">
                            ...
                            <div class="form-group">
                                ...
                                $row = $dataTypeRows->where('field', 'FichierFinance')->first();
                                @endphp
                                {!! app('voyager')->formField($row, $dataType, $dataTypeContent) !!}
                            </div>
                            <div class="form-group">
                                <label for="TypeFinance">Type de document</label>
                                <select class="form-control" name="TypeFinance">
                                    <option value="BUDGET" @if(isset($dataTypeContent->TypeFinance) &&
                                        $dataTypeContent->TypeFinance == 'BUDGET')
                                        selected="selected"@endif>الميزانية</option>
                                    <option value="COMPTEFINANCIER" @if(isset($dataTypeContent->TypeFinance) &&
                                        $dataTypeContent->TypeFinance ==
                                        'COMPTEFINANCIER') selected="selected"@endif>الحسابات المالبة</option>
                                    <option value="DETTES" @if(isset($dataTypeContent->TypeFinance) && $dataTypeContent->TypeFinance
                                        == 'DETTES') selected="selected"@endif>الديون
                                    </option>
                                    <option value="RECOUVREMENT" @if(isset($dataTypeContent->TypeFinance) && $dataTypeContent->TypeFinance
                                        == 'RECOUVREMENT') selected="selected"@endif>التغطية
                                    </option>
                                    <option value="RESULTATPERFO" @if(isset($dataTypeContent->TypeFinance) && $dataTypeContent->TypeFinance
                                        == 'RESULTATPERFO') selected="selected"@endif>نتائج تقييم الأداء
                                    </option>
                                    <option value="PLANAPPELOFFRES" @if(isset($dataTypeContent->TypeFinance) && $dataTypeContent->TypeFinance
                                        == 'PLANAPPELOFFRES') selected="selected"@endif>جدول قيادة الصفقات العمومية
                                    </option>
                                </select>
                            </div>
                        </div>
