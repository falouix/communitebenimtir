...
<div class="container-fluid">
    ...
    <a href="{{ route('voyager.'.$dataType->slug.'.indexAttente')  }}" class="btn btn-warning btn-add-new">
        <i class="voyager-check"></i> <span>En attente d'approbation ({{ $nbreEnAttente }})</span>
        @if($nbreEnAttente>0)
        <img src="{{ voyager_asset('images/new.gif') }}" style="width:50px;height:20px;" />
        @endif

    </a>
    <a href="{{ route('voyager.'.$dataType->slug.'.indexArchive')  }}" class="btn btn-default btn-add-new">
        <i class="voyager-archive"></i> <span>Archives</span>
    </a>
    @endcan
        ...
        <div class="tab-content">
            <div class="collapsible">
                <div class="collapse-head" data-toggle="collapse" data-target="#links" aria-expanded="true"
                    aria-controls="links">
                    <i class="voyager-angle-down" style="display: none;"><b>Archivage par période</b></i>
                    <i class="voyager-angle-up" style=""><b>Archivage par période</b></i>
                </div>
                    ...
                    <div class="row">
                        <div class="panel panel-bordered">
                            <div class="panel-body">
                                ...
                                <form action="#" method="#">
                                    <div class="col-md-5">
                                        <input type="date" id="date_deb1" name="date_deb" class="form-control">
                                    </div>
                                    <div class="col-md-5">
                                        <input type="date" id="date_fin1" name="date_fin" class="form-control">
                                    </div>
                                    <input hidden value="true" name="archive">
                                    <div class="col-md-2">
                                        <button type="button" class="btn btn-warning btn-add-new" onclick="ShowModal()">
                                            <i class="voyager-archive"></i> <span>Archiver</span></button>
                                    </div>
                                </form>
                            </div>
                        </div>
                        ...
                        <div id="search-input">
                            <select id="search_key" name="key">
                                ...
                                <option value="{{ $key }}" @if($search->key == $key || (empty($search->key) && $key ==
                                    $defaultSearchKey)){{ 'selected' }}@endif>{{ ucwords(str_replace('_', ' ', $key)) }}
                                </option>
                                @endforeach
                            </select>
...
<div class="modal modal-warning fade" tabindex="-1" id="archive_modal" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal"
                    aria-label="{{ __('voyager::generic.close') }}"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title"><i class="voyager-trash"></i> Êtes-vous sûr de vouloir archiver ??</h4>
            </div>
            <div class="modal-footer">
                <form action="{{ route('voyager.'.$dataType->slug.'.index') }}" id="archive_form" method="GET">
                    {{ csrf_field() }}
                    <input id="date_deb" name="date_deb" hidden>
                    <input id="date_fin" name="date_fin" hidden>
                    <input hidden value="true" name="archive">
                    <input type="submit" class="btn btn-warning pull-right archive-confirm" value="Confirmer">
                </form>
                <button type="button" class="btn btn-default pull-right"
                    data-dismiss="modal">{{ __('voyager::generic.cancel') }}</button>
            </div>
