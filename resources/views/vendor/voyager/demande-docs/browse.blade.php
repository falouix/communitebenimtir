...
<div class="container-fluid">
    ...
    <form action="{{ route('voyager.'.$dataType->slug.'.ChercherParPeriodeDocs') }}" method="GET"
...
<!--/start EtatDemandeDocs modal -->
{{-- EtatDemande_modal modal --}}
<div class="modal modal-warning fade" tabindex="-1" id="EtatDemande_modal" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('voyager.'.$dataType->slug.'.index') }}" id="EtatDemandeDocs_form" method="GET">
                {{ csrf_field() }}
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal"
                        aria-label="{{ __('voyager::generic.close') }}"><span aria-hidden="true">&times;</span></button>
                ...
                <div class="modal-footer">
                    <input type="submit" class="btn btn-warning pull-right EtatDemandeDocs-confirm" value="Oui">
                    <button type="button" class="btn btn-default pull-right"
                        data-dismiss="modal">{{ __('voyager::generic.cancel') }}</button>
                </div>
            </form>
...
<script>
    $(document).ready(function () {
        ...
        //EtatDemandeDocs
        var EtatDemandeDocsFormAction;
        $('td').on('click', '.LivRefDemande', function (e) {
            var EtatTitle = this.title;
          // alert(EtatTitle);
            if(EtatTitle=="Livrer")
            {
            //alert(EtatTitle);
            $('form ').append('<input type="text" id="Livrer" name="Livrer" value="1" hidden>');
            }
            else
            {
              $('form #Livrer').remove()
            }
        $('#EtatDemandeDocs_form')[0].action = '{{ route('voyager.'.$dataType->slug.'.EtatDemandeDocs', ['id' => '__id']) }}'.replace('__id', $(this).data('id'));
