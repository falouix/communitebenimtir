...
<div class="page-content container-fluid">
    <form class="form-edit-add" role="form"
        action="@if(isset($dataTypeContent->id)){{ route('voyager.annuaire-etablissements.update', $dataTypeContent->id) }}@else{{ route('voyager.annuaire-etablissements.store') }}@endif"
                    ...
                    <div class="tab-content">
                        <div id="FR" class="tab-pane fade">
                            ...
                            <div class="form-group">
                                <label for="slug">Nom d'établissement</label>
                                <input type="text" class="form-control" id="libelle_fr" name="libelle_fr"
                                    placeholder="Nom d'établissement" value="{{ $dataTypeContent->libelle_fr ?? '' }}"
                                    autocomplete="off">
                            </div>
                            <div class="form-group">
                                <label for="slug">Adresse</label>
                                <input type="text" class="form-control" id="adresse_fr" name="adresse_fr"
                                    placeholder="L'adresse de l'établissement"
                                    value="{{ $dataTypeContent->adresse_fr ?? '' }}" autocomplete="off">
                            </div>
                            <div class="form-group">
                        ...
                        <div id="AR" class="tab-pane fade active in" style="direction: rtl;">
                            <!-- ### TITLE ### -->
                            <div class="form-group">
                                <label for="slug">إسم المؤسسة</label>
                                <input type="text" class="form-control" id="libelle_ar" name="libelle_ar"
                                    placeholder="إسم المؤسسة" value="{{ $dataTypeContent->libelle_ar ?? '' }}"
                                    autocomplete="off">
                            </div>
                            <div class="form-group">
                                <label for="slug">العنوان</label>
                                <input type="text" class="form-control" id="adresse_ar" name="adresse_ar"
                                    placeholder="العنوان" value="{{ $dataTypeContent->adresse_ar ?? '' }}"
                                    autocomplete="off">
                            </div>

                            <div class="form-group">
                                <label for="slug">تقديم المؤسسة</label>
                                @php
                                $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows'
                                )};
                        ...
                        <div id="EN" class="tab-pane fade">
                            <!-- ### TITLE ### -->
                            <div class="form-group">
                                <label for="slug">The name</label>
                                <input type="text" class="form-control" id="libelle_en" name="libelle_en"
                                    placeholder="The name" value="{{ $dataTypeContent->libelle_en ?? '' }}"
                                    autocomplete="off">
                            </div>
                            <div class="form-group">
                                <label for="slug">Adress</label>
                                <input type="text" class="form-control" id="adresse_en" name="adresse_en"
                                    value="{{ $dataTypeContent->adresse_en ?? '' }}" autocomplete="off">
                            </div>
                            <div class="form-group">
                                <label for="slug">Description</label>
                                @php
                        ...
                        <div id="images" class="tab-pane fade">
                            ...
                            <div class="form-group">
                                <label for="slug">Fax</label>
                                @php
                                $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows'
                                )};
                                $row = $dataTypeRows->where('field', 'fax')->first();
                                @endphp
                                {!! app('voyager')->formField($row, $dataType, $dataTypeContent) !!}
                            </div>
                            <div class="form-group">
                                <label for="slug">Email Responsable accès à l'infos</label>
                                @php
                                $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows'
                                )};
                                $row = $dataTypeRows->where('field', 'E_mail')->first();
                                @endphp
                                {!! app('voyager')->formField($row, $dataType, $dataTypeContent) !!}
                            </div>
                            <div class="form-group">
                                <label for="slug">Email de Contact</label>
                                @php
                                $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows'
                                )};
                                $row = $dataTypeRows->where('field', 'E_mailContact')->first();
                                @endphp
                                {!! app('voyager')->formField($row, $dataType, $dataTypeContent) !!}
                            </div>
                            <div class="form-group">
                                <label for="slug">Googlemaps marker</label>
                                @php
                                $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows'
                                )};
                                $row = $dataTypeRows->where('field', 'googlemaps_marker')->first();
                                @endphp
                                {!! app('voyager')->formField($row, $dataType, $dataTypeContent) !!}
                            </div>
                        </div>
                    ...
                    <div class="panel-body">
                        ...
                        <div class="form-group">
                            ...
                            <select name="featured" class="form-control">
                                <option value="1" @if(isset($dataTypeContent->featured) &&
                                    $dataTypeContent->featured == 1)
                                    selected="selected"@endif>Commissariat régional</option>
                                <option value="2" @if(isset($dataTypeContent->featured) &&
                                    $dataTypeContent->featured == 2)
                                    selected="selected"@endif>Direction régionale</option>
                                <option value="ARCHIVE" @if(isset($dataTypeContent->featured)
                                    && $dataTypeContent->featured== 3)
                                    selected="selected"@endif>Entreprise Sous Tutelle
                                </option>
                            </select>
