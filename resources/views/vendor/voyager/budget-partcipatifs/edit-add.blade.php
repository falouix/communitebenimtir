...
<div class="page-content container-fluid">
    <form class="form-edit-add" role="form"
        action="@if(isset($dataTypeContent->id)){{ route('voyager.budget-partcipatifs.update', $dataTypeContent->id) }}@else{{ route('voyager.budget-partcipatifs.store') }}@endif"
                ...
                <div class="page-content compass container panel panel panel-bordered panel-primary "
                    ...
                    <div class="tab-content">
                        ...
                        <div id="images" class="tab-pane fade">
                            <div class="form-group">
                                <label for="slug">Année</label>
                                <input type="text" class="form-control" id="Annee" name="Annee" placeholder="Annee"
                                    value="{{ $dataTypeContent->Annee ?? '' }}" autocomplete="off">
                            </div>
                            <div class="form-group">
                                <label for="slug">Le document</label>
                                @php
                                $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows' )};
                                $row = $dataTypeRows->where('field', 'Document')->first();
                                @endphp
                                {!! app('voyager')->formField($row, $dataType, $dataTypeContent) !!}
                            </div>
                            <div class="form-group">
                                <label for="TypeBudget">Type de document</label>
                                <select class="form-control" name="TypeBudget">
                                    <option value="DIAGNOSTIQUE_TECH_FINANCIER" @if(isset($dataTypeContent->TypeBudget) &&
                                        $dataTypeContent->TypeBudget == 'DIAGNOSTIQUE_TECH_FINANCIER')
                                        selected="selected"@endif>التشخيص الفني و المالي</option>
                                    <option value="ELABORATION_PROGRAMME" @if(isset($dataTypeContent->TypeBudget) &&
                                        $dataTypeContent->TypeBudget =='ELABORATION_PROGRAMME') selected="selected"@endif>
                                        إعداد البرنامج التشاركي</option>
                                    <option value="BUDGET_PARTICIPATIF" @if(isset($dataTypeContent->TypeBudget) && $dataTypeContent->TypeBudget
                                        == 'BUDGET_PARTICIPATIF') selected="selected"@endif>الميزانية التشاركية
                                    </option>
                                    <option value="COMMISSION_PAR_ARRONDISSEMENT" @if(isset($dataTypeContent->TypeBudget) && $dataTypeContent->TypeBudget
                                        == 'COMMISSION_PAR_ARRONDISSEMENT') selected="selected"@endif>الجلسات التشاركية بالمناطق
                                    </option>
                                    <option value="COMMISIONS_PARTICIPATIFS" @if(isset($dataTypeContent->TypeBudget) && $dataTypeContent->TypeBudget
                                        == 'COMMISIONS_PARTICIPATIFS') selected="selected"@endif>الجلسات التشاركية
                                    </option>
                                </select>
                            </div>
                        </div>
