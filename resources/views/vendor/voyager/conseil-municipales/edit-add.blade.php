...
<div class="page-content container-fluid">
    <form class="form-edit-add" role="form"
        action="@if(isset($dataTypeContent->id)){{ route('voyager.conseil-municipales.update', $dataTypeContent->id) }}@else{{ route('voyager.conseil-municipales.store') }}@endif"
                ...
                <div class="page-content compass container panel panel panel-bordered panel-primary "
                    ...
                    <div class="tab-content">
                        <div id="FR" class="tab-pane fade active in">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="titre_fr" name="titre_fr"
                                    placeholder="Le titre de conseil" value="{{ $dataTypeContent->titre_fr ?? '' }}"
                                    autocomplete="off">
                            </div>
                        </div>
                        <div id="AR" class="tab-pane fade" style="direction: rtl;">
                            <!-- ### TITLE ### -->
                            <div class="form-group">
                                <label for="slug">إسم الجلسة</label>
                                <input type="text" class="form-control" id="titre_ar" name="titre_ar"
                                    placeholder="اكتب إسم الجلسة" value="{{ $dataTypeContent->titre_ar ?? '' }}"
                                    autocomplete="off">
                            </div>
                        </div>
                        <div id="images" class="tab-pane fade">
                            ...
                            <div class="form-group">
                                <label for="slug">Pv de réunion</label>
                                @php
                                $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows' )};
                                $row = $dataTypeRows->where('field', 'FichierConseil')->first();
                                @endphp
                                {!! app('voyager')->formField($row, $dataType, $dataTypeContent) !!}
                            </div>
                            <div class="form-group">
                                <label for="TypeConseil">Type de réunion</label>
                                <select class="form-control" name="TypeConseil">
                                    <option value="PREP" @if(isset($dataTypeContent->TypeConseil) &&
                                        $dataTypeContent->TypeConseil == 'PREP')
                                        selected="selected"@endif>تمهيدية</option>
                                    <option value="ORDINAIRE" @if(isset($dataTypeContent->TypeConseil) &&
                                        $dataTypeContent->TypeConseil ==
                                        'ORDINAIRE') selected="selected"@endif>عادية</option>
                                    <option value="EXTRA" @if(isset($dataTypeContent->TypeConseil) &&
                                        $dataTypeContent->TypeConseil
                                        == 'EXTRA') selected="selected"@endif>إستثنائية
                                    </option>
                                    <option value="GENERAL" @if(isset($dataTypeContent->TypeConseil) &&
                                        $dataTypeContent->TypeConseil
                                        == 'GENERAL') selected="selected"@endif>عامة
                                    </option>
                                </select>
                            </div>
                        </div>
            ...
            <div class="col-md-4">
                ...
                <div class="panel panel panel-bordered panel-warning">
                    ...
                    <div class="panel-body">
                        ...
                        <div class="form-group">
                            <label for="slug">Date de publication</label>
                            @php
                            $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows' )};
                            $row = $dataTypeRows->where('field', 'date_publication')->first();
                            if (isset($dataTypeContent->id)==false){$dataTypeContent->date_publication=date('d-m-Y');}
