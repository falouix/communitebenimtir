...
<div class="container-fluid">
    ...
    {{--Recherche pa etat citoyens--}}
    <form action="{{ route('voyager.'.$dataType->slug.'.ChercherParEtat') }}" method="GET" enctype="multipart/form-data"
        role="form">
        {{ csrf_field() }}
        <h4>Chercher par Etat de compte</h4>
        <div class="col-md-3">
...
<!--/start EditEtat modal -->
{{-- EditEtat_modal modal --}}
<div class="modal modal-warning fade" tabindex="-1" id="EditEtat_modal" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('voyager.'.$dataType->slug.'.index') }}" id="EditEtat_form" method="GET">
                {{ csrf_field() }}
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal"
                        aria-label="{{ __('voyager::generic.close') }}"><span aria-hidden="true">&times;</span></button>
                ...
                <div class="modal-body">
                    ...
                    <textarea class="form-control" id="raison" name="raison" required="true"></textarea>
                </div>
                <div class="modal-footer">
                    <input type="submit" class="btn btn-warning pull-right EditEtat-confirm" value="Oui">
                    <button type="button" class="btn btn-default pull-right"
                        data-dismiss="modal">{{ __('voyager::generic.cancel') }}</button>
                </div>
            </form>
...
<script>
    $(document).ready(function () {
        ...
        //EditEtat
        var EditEtatFormAction;
        $('td').on('click', '.EditEtat', function (e) {
            var EtatTitle = this.title;
            //alert(name);
            if(EtatTitle!="Suspendu")
            {
                //alert(EtatTitle);
                $( ".modal-body" ).hide();
        ...
        $('#EditEtat_form')[0].action = '{{ route('voyager.'.$dataType->slug.'.ModifEtat', ['id' => '__id']) }}'.replace('__id', $(this).data('id'));
        $('#EditEtat_modal').modal('show');
        });
