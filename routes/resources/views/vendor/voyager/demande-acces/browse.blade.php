...
<div class="container-fluid">
    ...
    <form action="{{ route('voyager.'.$dataType->slug.'.ChercherParPeriode') }}" method="GET"
        enctype="multipart/form-data" role="form">
        {{ csrf_field() }}
        <h4>Chercher par Période</h4>
        <div class="col-md-3">
        ...
        <div class="col-md-3">
            <div class="form-group">
                <select class="form-control" name="etat">
                    <option value="3">Tous les états</option>
                    <option value="0">En attente</option>
                    <option value="1">Satisfaite</option>
                    <option value="2">Rejeté</option>
                </select>
            </div>
        </div>
        <div class="col-md-3">
            <button type="submit" class="btn btn-primary" name="valider">Rechercher</button> </div>
    </form>
</div>
@stop
...
<!--/start EtatDemandeAcces modal -->
{{-- EtatDemande_modal modal --}}
<div class="modal modal-warning fade" tabindex="-1" id="EtatDemande_modal" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('voyager.'.$dataType->slug.'.index') }}" id="EtatDemandeAcces_form" method="GET">
                {{ csrf_field() }}
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal"
                        aria-label="{{ __('voyager::generic.close') }}"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="voyager-trash"></i> êtes-vous sûr(e) ?</h4>

                </div>
                <div class="modal-footer">
                    <input type="submit" class="btn btn-warning pull-right EtatDemandeAcces-confirm" value="Oui">
                    <button type="button" class="btn btn-default pull-right"
                        data-dismiss="modal">{{ __('voyager::generic.cancel') }}</button>
                </div>
            </form>
...
<script>
    $(document).ready(function () {
        ...
        //EtatDemandeAcces
        var EtatDemandeAccesFormAction;
        $('td').on('click', '.Satisfaite', function (e) {
            var EtatTitle = this.title;
          // alert(EtatTitle);
            if(EtatTitle=="Marquer comme non satisfaite")
            {
            //alert(EtatTitle);
            $('form ').append('<input type="text" id="MarquerNonSatisfaite" name="MarquerNonSatisfaite" value="1" hidden>');
            }
            else
            {
              $('form #MarquerNonSatisfaite').remove()
            }
        $('#EtatDemandeAcces_form')[0].action = '{{ route('voyager.'.$dataType->slug.'.EtatDemandeAcces', ['id' => '__id']) }}'.replace('__id', $(this).data('id'));
        $('#EtatDemande_modal').modal('show');
        });
