...
<div class="page-content container-fluid">
    <form class="form-edit-add" role="form"
        action="@if(isset($dataTypeContent->id)){{ route('voyager.pub-recherches.update', $dataTypeContent->id) }}@else{{ route('voyager.pub-recherches.store') }}@endif"
                ...
                <div class="page-content compass container panel panel panel-bordered panel-primary "
                    ...
                    <div class="tab-content">
                        ...
                        <div id="AR" class="tab-pane fade  active in" style="direction: rtl;">
                            <!-- ### TITLE ### -->
                            <div class="form-group">
                                <label for="slug">عنوان البحث / المنشور</label>
                                <input type="text" class="form-control" id="titre_ar" name="titre_ar"
                                    placeholder="اكتب عنوان البحث" value="{{ $dataTypeContent->titre_ar ?? '' }}"
                                    autocomplete="off">
                            </div>
                            <div class="form-group">
                                <label for="slug">نص البحث</label>
                                @php
                                $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows'
                                )};
                        ...
                        <div id="images" class="tab-pane fade">
                            ...
                            <div class="form-group">
                                <label for="slug">Fichier de la publication / recherche</label>
                                @php
                                $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows' )};
                                $row = $dataTypeRows->where('field', 'Fichier')->first();
                                @endphp
                                {!! app('voyager')->formField($row, $dataType, $dataTypeContent) !!}
                            </div>
                        </div>
                ...
                <div class="panel panel panel-bordered panel-warning">
                    ...
                    <div class="panel-body">
                        ...
                        <div class="form-group">
                            ...
                            @php
                            $dataTypeRows = $dataType->{(isset($dataTypeContent->id) ? 'editRows' : 'addRows' )};
                            $row = $dataTypeRows->where('field', 'DatePublication')->first();
                            if (isset($dataTypeContent->id)==false){$dataTypeContent->DatePublication=date('d-m-Y');}
                            @endphp
                            {!! app('voyager')->formField($row, $dataType, $dataTypeContent) !!}
                        </div>
