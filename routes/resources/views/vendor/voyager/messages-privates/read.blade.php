        ...
        <div class="col-md-12">
            <div class="panel panel-bordered" style="padding-bottom:5px;">
                ...
                <div class="panel-body">
                    <div class="card card-rec card-outline">
                        ...
                        <div class="card-body p-0 pad-top-0">
                            <div class="mailbox-read-info">
                                <h5>Sujet : {{ $MessagPrivee->Sujet }}</h5>
                                <h6>De <b class="fw-600">{{$MessagPrivee->Expediteur}}</b>
                                    <span class="mailbox-read-time float-right">{{$MessagPrivee->DateEnvoie}}</span>
                                </h6>
                            </div>
                            <!-- /.mailbox-read-info -->
                            <div class="space"></div>
                            ...
                            <div class="mailbox-read-message">
                                <p>{!! $MessagPrivee->Textmessage !!} </p>
                            </div>
                            <!-- /.mailbox-read-message -->
                        </div>
                        <!-- /.card-body -->
                        @if(!empty($MessagPrivee->PiecesJointes))
                        <div class="card-footer bg-white">
                            @foreach(json_decode($MessagPrivee->PiecesJointes, true) as $fichier)
                            @php
                    ...
                    @foreach ( $ListReponseMessagPrivee as $reponseMessagPrivee )
                    <div class="card card-primary card-outline">
                        <!-- /.card-header -->
                        <div class="card-body p-0 pad-top-0">
                            <div class="mailbox-read-info">
                                <h6>De {{$reponseMessagPrivee->Expediteur}}
                                    <span
                                        class="mailbox-read-time float-right">{{$reponseMessagPrivee->DateReponse}}</span>
                                </h6>
                            </div>
                            <!-- /.mailbox-read-info -->
                            <div class="space"></div>
                            <!-- /.mailbox-controls -->
                            <div class="mailbox-read-message">
                                <p>{!! $reponseMessagPrivee->TextReponse !!} </p>
                            </div>
                            <!-- /.mailbox-read-message -->
                        </div>
                        <!-- /.card-body -->
                        @if(!empty($reponseMessagPrivee->PiecesJointes))
                        <div class="card-footer bg-white">
                            @foreach(json_decode($reponseMessagPrivee->PiecesJointes, true) as $fichier)
                            @php
                            $split = explode("/", $fichier);
                            @endphp
                            <div class="card-body pad-top-0 bg-white">
            ...
            <div class="panel panel-bordered panel-info">
                ...
                <div class="panel-body" style="">
                    <form action="{{ route('voyager.'.$dataType->slug.'.RepondreMessage') }}" method="POST"
                        ...
                        <input type="hidden" name="messageprivee" id="messageprivee" value="{{$MessagPrivee->id}}">
                    </form>
                </div>
            </div>
        </div>
