...
<h1 class="page-title">
    ...
    <div class="btn btn-success add_item"><i class="voyager-plus"></i> {{ __('voyager::menu_builder.new_menu_item') }}
    </div>
</h1>
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><i class="voyager-trash"></i>
                    {{ __('voyager::menu_builder.delete_item_question') }}</h4>
            </div>
        ...
        <div class="modal-content">
            ...
            <form action="" id="m_form" method="POST"
                ...
                <div class="modal-body">
                    ...
                    <!--ajouter titre enp-->
                    <label for="name">Title for the menu item</label>
                    @include('voyager::multilingual.input-hidden', ['_field_name' => 'title_en', '_field_trans' => ''])
                    <input type="text" class="form-control" id="m_title_en" name="title_en"
                        placeholder="{{ __('voyager::generic.title') }}"><br>
                    <!--ajouter titre ar -->
                    <label for="name">عنوان عنصر القائمة</label>
                    @include('voyager::multilingual.input-hidden', ['_field_name' => 'title_ar', '_field_trans' => ''])
                    <input type="text" class="form-control" id="m_title_ar" name="title_ar"
                        placeholder="{{ __('voyager::generic.title') }}"><br>
                    <!--Ajouter pages-->
                    <label for="target">Choisir une page</label>
                    <select id="m_page" class="form-control" name="page" onchange="set_url()">
                        <option value=""> Choisir une page</option>
                        @foreach($pages as $item )
                        <option value="{{ $item->slug }}"> {{ $item->titre_ar }}</option>
                        @endforeach
                    </select><br>
                    <!--/Ajouter pages-->
                    <div id="m_url_type">
...
<script>
    function set_url()
    {
        var page= document.getElementById("m_page").value;
        document.getElementById("m_url").value = "/pages/"+page;
        //alert(page);
    }
        $(document).ready(function () {
            ...
            var $m_modal       = $('#menu_item_modal'),
                ...
                $m_title       = $('#m_title'),
                $m_title_ar       = $('#m_title_ar'),
                $m_title_en       = $('#m_title_en'),
            ...
            $m_modal.on('show.bs.modal', function(e, data) {
                ...
                if (_adding) {
                    ...
                    $m_title.val(_src.data('title'));
                     $m_title_en.val(_src.data('title_en'));
                      $m_title_ar.val(_src.data('title_ar'));
