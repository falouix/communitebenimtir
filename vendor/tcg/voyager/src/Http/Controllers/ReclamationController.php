...
class ReclamationController extends Controller
{
    ...
    public function EtatRecl(Request $request, $id)
    {
        ...
        //dd($user);
        $idReclamation=$id;
        $reclamation = Reclamation::where('id', $idReclamation)->first();
        //dd($reclamation->Etat);
        //En attente
        if($reclamation->Etat==0)
        {
            $reclamation->Etat =1;
            //$reclamation->ConfirmePar=$CurrentUser->name;
        }
        $reclamation->save();
    ...
    **********************************************************/
    public function ConsulterReclamation(Request $request, $id)
    {
        $slug = $this->getSlug($request);
        //dd($slug);
        $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();
        $idReclamation=$id;
        $reclamation = Reclamation::where('id', $idReclamation)->first();
        $idCitoyen=$reclamation->IdCitoyen;
        $citoyen = Citoyen::where('id', $idCitoyen)->first();
        $ListReponseReclamation = ReponseReclamation::where('Id_reclamation', $idReclamation)->get();
        //dd($reclamation);
        $slug = $this->getSlug($request);
        $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();
        $isSoftDeleted = false;
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted','reclamation','ListReponseReclamation','citoyen'));
    }
    /*********************************************************
                   Répondre Reclamation
    **********************************************************/
    public function RepondreReclamation(Request $request)
    {
        $slug = $this->getSlug($request);
        $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();
        $CurrentUser = auth()->user();
...
$string_fichiers .="]";
    ...
    public function ChercherParPeriode(Request $request)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            elseif($etat=="3" && !empty($dat_deb) &&  !empty($date_fin)) {
                $query =$model::whereBetween('DateReclamation', [$dat_deb, $date_fin]);
            }
            elseif($etat!="3" && empty($dat_deb) &&  empty($date_fin)) {
